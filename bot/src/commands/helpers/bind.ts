...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption((option) =>
    option
      .setName("key")
      .setDescription("The binding key you received from the bot")
      .setRequired(true)
  );
...
export const execute = async (
  ...
  if (!valid.success) {
    await interaction.reply(
      errorEmbedReply("The binding key you provided is invalid")
    );
    return;
  }
  ...
  if (valid.data) {
    ...
    if (binded) {
      await interaction.reply(
        successEmbedReply("Your account has been successfully binded")
      );
      return;
    }
  }
  ...
  await interaction.reply(
    errorEmbedReply("Something wen't wrong binding your account")
  );
};
